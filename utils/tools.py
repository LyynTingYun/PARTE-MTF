...
def parse_setting(setting):
    """
    Example:
    finetune_PARTE_ETTh1_M_sl336_ll0_pl96_dm16_df64...
    ...
    pattern = r'^finetune_(.*?)_(.*?)_M_.*?sl(\d+).*?pl(\d+)'

    match = re.search(pattern, setting)

    if match is None:
        raise ValueError(f"Cannot parse setting:\n{setting}")

    return {
        "model": match.group(1),
        "dataset": match.group(2),
        "sequence_length": int(match.group(3)),
        "prediction_length": int(match.group(4))
    }
